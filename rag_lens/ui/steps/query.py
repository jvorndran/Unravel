"""Query tester page for RAG Lens.
...
from rag_lens.services.embedders import DEFAULT_MODEL, get_embedder
...
def _get_llm_config_from_sidebar() -> tuple[LLMConfig, str]:
    ...
    from rag_lens.services.llm import get_api_key_from_env
...
def render_query_step() -> None:
    ...
    provider = st.session_state.get("llm_provider", "OpenAI")
    from rag_lens.services.llm import LLM_PROVIDERS, get_api_key_from_env
    ...
    if ask_clicked and query_text.strip():
        ...
        # Validate LLM config
        from rag_lens.services.llm import validate_config
        ...
        if enable_query_expansion:
            from rag_lens.services.llm import rewrite_query_variations
        ...
        with st.spinner("Retrieving context..."):
            ...
            from rag_lens.services.retrieval.reranking import (
                RerankerConfig,
                rerank_results,
            )
            ...
            if reranking_config.get("enabled", False) and search_results:
                with st.spinner("Reranking results..."):
                    try:
                        ...
                        st.warning("FlashRank not installed. Install with: pip install rag-lens[reranking]")
                    except Exception as e:
        ...
        with response_container:
            ...
            with st.container(border=True):
                ...
                try:
                    ...
                    st.info("Install the required library with: `pip install rag-lens[llm]`")
                except Exception as e:
